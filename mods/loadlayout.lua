...
function setRipple(v,c)
  ...
  ripple_bounded.setColor(ColorStateList(int[0]{int[0]},int{c}))
  v.setForeground(ripple_bounded)
end

function setUnRipple(v,c)
  import "android.content.res.ColorStateList"
  ripple_unbounded = activity.getResources().getDrawable(R.drawable.ripple_unbounded)
  ...
  if k=="layout_x" then
   ...
   elseif k=="layout_behavior" then
    ...
    if tostring(v) == "@string/appbar_scrolling_view_behavior" then
      ...
      local ScrollingViewBehavior = import "android.support.design.widget.AppBarLayout$ScrollingViewBehavior"

      params.setBehavior(ScrollingViewBehavior())

     elseif tostring(v) == "@string/bottom_sheet_behavior" then

      local BottomSheetBehavior = import "android.support.design.widget.BottomSheetBehavior"
   ...
   elseif k=="url" then
    view.loadUrl(url)
   elseif k=="src" then
    if luajava.instanceof(view,SvgView) then
      view.setImageURI(activity.getUriForPath(v))
      return
    end
    if luajava.instanceof(view,LuaView) then
      view.setRunPath(v)
      return
    end
    if v:find("^%?") then
